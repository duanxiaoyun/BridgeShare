...
public class GameController : MonoBehaviour {
    ...
    IEnumerator WaitShowResult(float time)
    {
        ...
        if (gameData.totalScore >= GameArchive.jumpRecord.GetHighScore() && LevelManager.currentLevel==LevelName.Jump)/*GameArchive.jumpRecord.IsHighRecord()*/
        {
            gameEffs[2].SetActive(true);
            yield return new WaitForSeconds(2.0f);// 播放游戏的highscore的动画效果。
            gameEffs[2].SetActive(false);
        }
        if (gameData.totalScore >= GameArchive.pushBallRecord.GetHighScore() && LevelManager.currentLevel == LevelName.PushBall)/*GameArchive.pushBallRecord.IsHighRecord()*/
        {
            gameEffs[2].SetActive(true);
            yield return new WaitForSeconds(2.0f);// 播放游戏的highscore的动画效果。
            gameEffs[2].SetActive(false);
